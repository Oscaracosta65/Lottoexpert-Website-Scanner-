...
namespace LottoExpert\Component\LeSiteAudit\Administrator\Controller;
...
// ---------------------------------------------------------------------------
// Helper functions — global in this namespace; called by the controller below.
// All leAudit* functions are adapted from the original Sourcerer-embedded
// article. No logic changes: only the session API and base URL have been
// updated for a native Joomla component.
// ---------------------------------------------------------------------------
...
class DisplayController extends BaseController
{
    ...
    /**
     * Handle all requests: action dispatch + view rendering.
     *
     * All audit actions (scan_batch_ajax, discover_sitemap, export_csv, etc.)
     * are carried in the POST field "audit_action" to avoid conflicts with
     * Joomla's own "task" routing. JSON and CSV responses short-circuit here
     * with exit(); the normal page display falls through to parent::display().
     */
    public function display($cachable = false, $urlparams = []): static
    {
        ...
        if ($action === 'save_options' || $action === 'save_options_ajax') {
            ...
            leAuditSessionSet('le_queue', $queue);
            $message = 'Scan stopped. Results collected so far are preserved. Click Reset Audit to start a completely fresh scan.';
        ...
        // ── Set view properties ──────────────────────────────────────────────
        $view = $this->getView('scanner', 'html');

        $view->baseUrl        = Route::_('index.php?option=com_lesiteaudit');
